...
params.esm_embedding_dir = "esm_embedding_dir"
params.pdb_dir = "pdb_dir"
...
process binary_prediction {
    ...
    """
    python3 ${projectDir}/prediction_bin.py --graphData ${graph} --model ${model} --idx_maps_id ${index_maps_ID} --id_maps_prot_des ${id_maps_protein_description} --output_prefix ${output_prefix}
    """
...
workflow {
    ...
    //esm_embedding_dir_ch = Channel.from(params.esm_embedding_dir)
    //pdb_dir_ch = Channel.from(params.pdb_dir)
    ...
    binary_prediction(graph_ch.graph, params.path_to_bin_model, graph_ch.index_maps_ID, protein_fasta_with_ID_ch.protein_ID_maps_original_description, params.out_prefix)
